"""SPOTlight spatial deconvolution wrapper (optional)."""
...
class SPOTlightWrapper(BenchmarkMethod):
    name = "SPOTlight"
    modality = "spatial"
    ...
    def is_available(self) -> bool:
        return (_env.python_module_available("spotlight")
                or _env.r_package_available("SPOTlight"))

    def install_hint(self) -> str:
        return _env.INSTALL_HINTS.get("spotlight", "see method documentation")

    def _run(self, scenario: dict) -> pd.DataFrame:
        export_spatial_inputs(scenario, self.name)
        raise NotImplementedError(
            "SPOTlight is installed but automatic execution is not wired in this "
