...
Sweeps the CAR spatial smoothing strength lambda_spatial over a grid on a
structured synthetic spatial scenario (sharp border + gradient + rare niche +
mixed/collinear spots), replicated across seeds, for BREAST and HLCA/LUNG.
...
LUNG_H5AD = REPO / "examples" / "second_tissue_lung" / "data" / "derived" / "hlca_subset.h5ad"
...
# Per-dataset configuration. domain_families map the L / R / gradient domains to
# tissue-specific broad families; rare_candidates are rare fine types used for
# the niche (first present in the reference wins).
DATASET_CFG = {
    "breast": {
        "h5ad": H.REFERENCE_H5AD,
        "celltype_col": None,            # auto-detected by H.prepare_reference
        "donor_col": "donor_id",
        "hierarchy_tsv": BREAST_HIERARCHY,
        "domain_families": None,         # breast default layout
        "rare_candidates": ["regulatory T cell", "natural killer cell"],
    },
    "lung": {
        "h5ad": LUNG_H5AD,
        "celltype_col": "cell_type_fine",
        "donor_col": "donor_id",
        "broad_col": "cell_type_broad",  # mapping read directly from obs
        "domain_families": {
            "left": ["Airway epithelium", "Alveolar epithelium"],
            "right": ["Lymphoid", "Myeloid"],
            "gradient": ["Fibroblast lineage", "Blood vessels", "Smooth muscle"],
        },
        "rare_candidates": ["NK cells", "Neuroendocrine", "Tuft"],
    },
}
...
def _load_dataset(name, *, export_for_external=False):
    """Build the reference + scenario config for a dataset. Returns a dict."""
    ...
    ref_donors, query_donors = SH.split_donors(adata, donor_col, ref_frac=0.5, seed=0)
    ref_mask = adata.obs[donor_col].astype(str).isin(set(ref_donors)).to_numpy()
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        ref = H.prepare_reference(adata[ref_mask].copy(), min_cells=MIN_CELLS,
                                  cell_type_col=cfg["celltype_col"],
                                  estimate_overdispersion=True).reference
    ref_types = [str(c) for c in ref.cell_types]

    if cfg.get("hierarchy_tsv"):
        mapping = build_cell_type_hierarchy(ref_types, load_hierarchy_mapping(cfg["hierarchy_tsv"]))
        celltype_col = "cell_type"
    else:
        ct, bc = cfg["celltype_col"], cfg["broad_col"]
        m = adata.obs[[ct, bc]].astype(str).drop_duplicates()
        full = dict(zip(m[ct], m[bc]))
        mapping = {t: full.get(t, t) for t in ref_types}
        celltype_col = ct

    rare_type = next((t for t in cfg["rare_candidates"] if t in ref_types), ref_types[-1])
    return {"adata": adata, "ref": ref, "ref_types": ref_types, "mapping": mapping,
            "celltype_col": celltype_col, "donor_col": donor_col,
            "query_donors": query_donors, "ref_donors": ref_donors,
            "rare_type": rare_type, "domain_families": cfg["domain_families"]}


def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description=__doc__)
    ap.add_argument("--run-real-data", action="store_true")
    ap.add_argument("--datasets", nargs="+", choices=["breast", "lung"], default=["breast", "lung"])
    ...
    per_scenario, runtime_rows = [], []

    for ds_name in args.datasets:
        cfg = DATASET_CFG.get(ds_name)
        if not Path(cfg["h5ad"]).exists():
            print(f"[skip] {ds_name}: missing {cfg['h5ad']}")
            continue
        D = _load_dataset(ds_name)
        print(f"[{ds_name}] reference: {len(D['ref_types'])} types; rare={D['rare_type']}; "
              f"families={D['domain_families'] or 'breast-default'}")
        for seed in args.seeds:
            sc = SSP.generate_spatial_scenario(
                ...
                n_side=args.n_side, cells_per_spot=CELLS_PER_SPOT, seed=seed,
            ...
            for lam in args.lambdas:
                ...
                tcfg.spatial_solver.lambda_spatial = float(lam)
                ...
                try:
                    with warnings.catch_warnings(record=True) as caught:
                        ...
                        res = tr.deconv_spatial(
                            ...
                            sc["gene_names"], spot_ids=sc["spot_ids"], config=tcfg,
                            resolution_mode="none", run_neighbourhood=False)
                        n_warn = len(caught)
                    pred = res.deconv.proportions
                except Exception as exc:  # noqa: BLE001
                    print(f"  {ds_name} seed={seed} lambda={lam} FAILED: {exc}")
                    runtime_rows.append({"dataset": ds_name, "seed": seed, "lambda_spatial": lam,
                                         "runtime_seconds": float(time.perf_counter() - t0),
                                         "n_warnings": n_warn, "status": "failed"})
                    continue
                rt = float(time.perf_counter() - t0)
                preset = ("default" if lam == DEFAULT_LAMBDA else "weak_smoothing" if lam == 0.02
                          else "no_smoothing" if lam == 0.0 else f"grid({lam})")
                m = _score(truth, pred, coords, domains, D["mapping"], D["rare_type"])
                per_scenario.append({"dataset": ds_name, "seed": seed, "lambda_spatial": lam,
                                     "preset": preset, "is_default": lam == DEFAULT_LAMBDA,
                                     "runtime_seconds": round(rt, 2), "n_warnings": n_warn, **m})
                runtime_rows.append({"dataset": ds_name, "seed": seed, "lambda_spatial": lam,
                                     "runtime_seconds": round(rt, 2), "n_warnings": n_warn,
                                     "status": "ok"})
                print(f"  {ds_name} seed={seed} lambda={lam:<4} {rt:5.1f}s fine_r={m['fine_pearson']:.3f} "
    ...
    metric_cols = [c for c in ps.columns if c not in
                   ("dataset", "seed", "lambda_spatial", "preset", "is_default")]
    grid = ps.groupby(["dataset", "lambda_spatial"])[metric_cols].mean().reset_index()
    ...
    grid["n_seeds"] = ps.groupby(["dataset", "lambda_spatial"]).size().values
    grid.to_csv(OUT / "lambda_grid_metrics.tsv", sep="\t", index=False)

    pf = (ps.groupby(["dataset", "lambda_spatial"])[["conditional_rmse"]].mean()
    ...
    (OUT / "manifest.json").write_text(json.dumps({
        ...
        "datasets": args.datasets, "default_changed": False,
        "experimental_preset": "weak_smoothing (lambda=0.02)",
        "scenario": "sharp border + gradient + rare niche + mixed/collinear spots",
    }, indent=2), encoding="utf-8")
    print(f"\nWrote -> {OUT}/")
    show = ["dataset", "lambda_spatial", "preset", "fine_pearson", "broad_pearson", "local_rmse",
